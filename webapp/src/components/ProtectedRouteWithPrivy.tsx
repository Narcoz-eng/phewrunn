...
function getPrivyHandoffLabel(
  state: ReturnType<typeof getAuthUiState>,
  detail: string | null,
...
export function ProtectedRouteWithPrivy({
  children,
  ...
  const recentlyLoggedOut = logoutCooldownActive || isRecentExplicitLogoutSuppressed();
  const privySyncFailure = !effectiveUser ? privySyncFailureSnapshot : null;
  const activeLoginIntent = !effectiveUser && !recentlyLoggedOut ? readPrivyLoginIntent() : null;
  ...
  const routeAuthStage = isPending
    ? "trying_to_connect"
  ...
  useEffect(() => {
    if ((effectiveUser && hasLiveSession) || isPending || !shouldHoldForRecovery) {
      if (!isPending) {
    ...
    const timer = window.setTimeout(
      ...
      hasPrivyHydrationHint || hasPrivySyncHint || hasOAuthReturnHint || shouldHoldForConfirmedSession
        ? 12_000
        : 4_000
    );
