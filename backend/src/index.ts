...
import { bundleCheckerRouter } from "./routes/bundle-checker.js";
...
function requiresStrictPrismaReadiness(path: string): boolean {
  return !canBypassStrictPrismaReadiness(path);
}

app.use("/api/*", async (c, next) => {
  if (!prismaReady) {
    const requiresStrictReadiness = requiresStrictPrismaReadiness(c.req.path);
    const readinessTimeoutMs = requiresStrictReadiness ? 12_000 : 4_000;
    try {
      await Promise.race([
...
// Keep the Privy-user -> auth-user cache short-lived so auth fast paths can
// absorb transient database pressure without holding stale profile or role data
// for long periods.
const PRIVY_AUTH_USER_CACHE_TTL_MS = 5 * 60 * 1000;
...
app.route("/api/bundle-checker", bundleCheckerRouter);
